/**
 * کامپوننت WelcomeCircle - دایره شیشه‌ای برای صفحه Welcome
 */

import { useState, useEffect } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { convert } from '../utils/conversions/index.js';
import { getUnitInfo, getUnitsByCategory, categories } from '../constants/units.js';
...
export function WelcomeCircle() {
  ...
  // تغییر خودکار دسته، واحد و مقدار
  useEffect(() => {
    const interval = setInterval(() => {
      // تغییر دسته
      const currentCategoryIndex = categories.findIndex((c) => c.key === category);
      const nextCategoryIndex = (currentCategoryIndex + 1) % categories.length;
      const nextCategory = categories[nextCategoryIndex].key;
      setCategory(nextCategory);

      // تغییر واحدها
      const units = getUnitsByCategory(nextCategory);
      if (units.length > 1) {
        setFromUnit(units[0].key);
        setToUnit(units[1].key);
      } else {
        setFromUnit(units[0].key);
        setToUnit(units[0].key);
      }

      // تغییر مقدار
      const randomValue = Math.floor(Math.random() * 200) + 10;
      setValue(randomValue);
    }, 1200); // تغییر سریع‌تر (1.2 ثانیه)
  ...
  return (
    <div className="w-full flex justify-center px-4 py-6">
      <motion.div
        ...
        className="relative"
      >
        {/* دایره شیشه‌ای با انیمیشن pulse */}
        <motion.div
          animate={{
            scale: [1, 1.05, 1],
            boxShadow: [
              `0 20px 60px ${colors.from}40, inset 0 0 60px ${colors.to}20`,
              `0 25px 70px ${colors.from}60, inset 0 0 70px ${colors.to}30`,
              `0 20px 60px ${colors.from}40, inset 0 0 60px ${colors.to}20`,
            ],
          }}
          transition={{
